...
Copyright 2021 Daniel Cagigas-Mu�iz (dcagigas@us.es)
...
//#define __VIDEO	// Uncomment if a Celular Automaton snapshot set or video is generated
...
typedef  int POSITION_TYPE;             // Used to implement the population simulation movement: cell swapping
...
const COUNT_TYPE TIME_STEPS	=	199;		    // Simulated days: 199 days of 2020 year are simulated
...
const int L            =   11;                  // Maximun distance cell movement
...
const int LOWER_LIMIT = 2;				// 0 and 1 ghost rows/columns are not computed
const int UPPER_LIMIT = N-2;				// N-1 and N-2 ghost rows/columns are not computed
...
const int ERROR = 1;                    // RETURNED VALUE IF ERRORS
const int OK = 0;
const int NA = -1;                      // Not available. To implement movement (cell swapping)
