...
This file is part of covid19_NY_OpenMP.

covid19_NY_OpenMP is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 3 of the License, or
any later version.

covid19_NY_OpenMP is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
GNU General Public License for more details.

You should have received a copy of the GNU General Public License
along with covid19_NY_OpenMP.  If not, see <https://www.gnu.org/licenses/>.
...
void swapCells (int begin, int end, CELL_TYPE *health_states_grid, PROBABILITY_TYPE *pa_, PROBABILITY_TYPE *f3_, PROBABILITY_TYPE *RC_, PROBABILITY_TYPE move_proportion, CELL_TYPE max_distance);
...
// This is specific to OpenMP and the machine where the program is run.
// Please change according to the machine number of threads or cores.
#define NTHREADS 8


unsigned int seed[NTHREADS]; 	// Seed for random uniform number output.
...
int main () {
    ...
    for (int i=0; i<NTHREADS; i++)
        seed[i] = (int)(i+1)*time(NULL);


	allocate_main_simulation_variables ();
	init_zero_grid(&timer[0 * N * N]); 
	init_zero_grid(&timer[1 * N * N]);
	init_zero_grid(&timer[2 * N * N]);
	init_zero_grid(&timer[3 * N * N]);
	init_zero_grid (grid);
	init_grid (grid);
	init_infected (grid);
	...
	for (t=1; t<=TIME_STEPS; t++) {
		//printf ("%d\n", t);
		set_hospitalization_and_dead_fraction (t, &uu, &kk);                                        // Step 0
		calculate_infections (grid, pb, f3, RC);                                                    // Step 1
		calculate_infection_transitions (grid, pb, pa);                                             // Step 2
		calculate_rest_state_transitions (grid, timer, t, uu, kk, isC, isH, isD, isI, isIa, isR);   // Step 3
		simulate_population_movements (grid, pa, f3, RC, MOVE_PROPORTION, L);                       // Step 4
		calculate_self_isolation_state_transitions (grid, AC, t);                                   // Step 5
		#ifdef __VIDEO
...
void init_grid (CELL_TYPE *health_states_grid) {	
	...
	for (i=LOWER_LIMIT; i<UPPER_LIMIT; i++) {
		for (j=LOWER_LIMIT; j<UPPER_LIMIT; j++) {
			p = r4_uniform_01(&seed[64/sizeof(int) *omp_get_thread_num()]);
...
void init_infected (CELL_TYPE *health_states_grid) {	
    ...
    #pragma omp parallel for private (cont, i, j)
	for (cont=0; cont<INIT_INFECTED; cont++) {
		i = random_uint (N, &seed[64/sizeof(int) *omp_get_thread_num()]);
		j = random_uint (N, &seed[64/sizeof(int) *omp_get_thread_num()]);
        if (grid[i][j] != I)
		    grid[i][j] = I;
        else
            cont--;
...
void init_pa_f3_RC (PROBABILITY_TYPE *pa_, PROBABILITY_TYPE *f3_, PROBABILITY_TYPE *RC_, CELL_TYPE *health_states_grid) {
	...
	ff1 = 1;
    #pragma omp parallel for private (i, j, r1, r2, f1, f2)
	for (i=LOWER_LIMIT; i<UPPER_LIMIT; i++) {
		for (j=LOWER_LIMIT; j<UPPER_LIMIT; j++) {
			if (grid[i][j] != E) {
				r1 = r4_uniform_01(&seed[64/sizeof(int) *omp_get_thread_num()]);
				...
				r2 = r4_uniform_01(&seed[64/sizeof(int) *omp_get_thread_num()]);
				// pa: probability of becoming asymptomatic:
				// f3: infectivity
				// RC: resitance/inmunity to the infection
				if (r2 <= AGE_0_4) {
					pa[i][j] = PA_0_4_YEARS;
					f3[i][j] = r4_uniform_01(&seed[64/sizeof(int) *omp_get_thread_num()]);
					f2 = Fage_below_60;
					RC[i][j] = ff1*f1*f2*r4_uniform_01(&seed[64/sizeof(int) *omp_get_thread_num()]);
				} else if (r2 <= AGE_5_14) {
...
void calculate_infections (CELL_TYPE *health_states_grid, PROBABILITY_TYPE *pb_, PROBABILITY_TYPE *f3_, PROBABILITY_TYPE *RC_) {
    ...
    #pragma omp parallel for private (i, j, ifx1, ifx2, ifx3, ifx4, ifx5, ifx6, ifx7, ifx8, ifx11, ifx22, ifx33, ifx44, ifx55, ifx66, ifx77, ifx88, irc, AA, BB)
	for (i=LOWER_LIMIT; i<UPPER_LIMIT; i++) {
		for (j=LOWER_LIMIT; j<UPPER_LIMIT; j++) {
			if (grid[i][j] == S) {
				pb[i][j] = 0;
	...
	for (i=LOWER_LIMIT; i<UPPER_LIMIT; i++) {
		for (j=LOWER_LIMIT; j<UPPER_LIMIT; j++) {
			if (grid[i][j] == S) {
				...
				if (pb[i][j] > r4_uniform_01(&seed[64/sizeof(int) *omp_get_thread_num()]) && pa[i][j] > r4_uniform_01(&seed[64/sizeof(int) *omp_get_thread_num()]) ) {
					grid[i][j] = Ia;
				} else if (pb[i][j] > r4_uniform_01(&seed[64/sizeof(int) *omp_get_thread_num()]) && pa[i][j] <= r4_uniform_01(&seed[64/sizeof(int) *omp_get_thread_num()]) ) {
					grid[i][j] = I;
				}
			}
		}
...
int i,j;
    ...
    COUNT_TYPE temp_isIa = isIa[t];
    COUNT_TYPE temp_isI = isI[t];
    COUNT_TYPE temp_isC = isC[t];
    COUNT_TYPE temp_isH = isH[t];
    COUNT_TYPE temp_isD = isD[t];
    COUNT_TYPE temp_isR = isR[t];


	#pragma omp parallel for private (i, j) reduction(+:temp_isIa) reduction(+:temp_isI) reduction(+:temp_isC) reduction(+:temp_isH) reduction(+:temp_isD) reduction(+:temp_isR)
	for (i=LOWER_LIMIT; i<UPPER_LIMIT; i++) {
		for (j=LOWER_LIMIT; j<UPPER_LIMIT; j++) {
			if (grid[i][j] == Ia) {
				//#pragma omp atomic
				//isIa[t] = isIa[t] + 1; 		// Current infected (asymptomatic) number in simulation
                temp_isIa = temp_isIa +1;
			...
			if (time[1][i][j] == T2) {
				if (r4_uniform_01(&seed[64/sizeof(int) *omp_get_thread_num()]) > uu) {
					grid[i][j] = R;
				} else {
					grid[i][j] = H;
					//#pragma omp atomic
			...
			if (time[2][i][j] == T3) {
				if (r4_uniform_01(&seed[64/sizeof(int) *omp_get_thread_num()]) > kk) {
					grid[i][j] = R;
				} else {
					grid[i][j] = D;
					//#pragma omp atomic
...
void swapCells (int begin, int end, CELL_TYPE *health_states_grid, PROBABILITY_TYPE *pa_, PROBABILITY_TYPE *f3_, PROBABILITY_TYPE *RC_, PROBABILITY_TYPE move_proportion, CELL_TYPE max_distance) {
    ...
    for (i=begin; i<end; i++) {
		for (j=LOWER_LIMIT+max_distance; j<UPPER_LIMIT-max_distance; j++) {
			if (r4_uniform_01 (&seed[64/sizeof(int) *omp_get_thread_num()]) > 1-move_proportion) { 
				// Swap 
				di = (1 + random_uint (max_distance, &seed[64/sizeof(int) *omp_get_thread_num()])) - (1 + random_uint (max_distance, &seed[64/sizeof(int) *omp_get_thread_num()]));
				dj = (1 + random_uint (max_distance, &seed[64/sizeof(int) *omp_get_thread_num()])) - (1 + random_uint (max_distance, &seed[64/sizeof(int) *omp_get_thread_num()]));
				// Do swapping
				temp1 = grid[i][j];
...
void simulate_population_movements (CELL_TYPE *health_states_grid, PROBABILITY_TYPE *pa_, PROBABILITY_TYPE *f3_, PROBABILITY_TYPE *RC_, PROBABILITY_TYPE move_proportion, CELL_TYPE max_distance) {
    ...
    // The grid is divided in several horizontal bands.
    int band_size = N / NTHREADS;
    //int band_size = (UPPER_LIMIT-LOWER_LIMIT) / NTHREADS;

    // These bands perform the cell swapp process inside their safety zone.
    #pragma omp parallel num_threads(NTHREADS)
    {
        int id = omp_get_thread_num();
        int begin = LOWER_LIMIT + id * band_size + max_distance;
        //int begin = id * band_size + max_distance;
        //int end = (id == NTHREADS - 1) ? begin + band_size - 2*max_distance - LOWER_LIMIT : begin + band_size - 2*max_distance;
        int end = (id == NTHREADS - 1) ? UPPER_LIMIT-max_distance : begin + band_size - 2*max_distance;
        //int end = (id == NTHREADS - 1) ? (N-1)-max_distance : begin + band_size - 2*max_distance;
        /*if (id == NTHREADS - 1)
            int end = begin + band_size - 2*max_distance - LOWER_LIMIT;
        else
            int end = begin + band_size - 2*max_distance;*/
        swapCells (begin, end, health_states_grid, pa_, f3_, RC_, move_proportion, max_distance);
    }

   #pragma omp barrier

    // Now security zones that divided the bands perform the cell swapping.
    if (NTHREADS > 1) {
        ...
        int id = omp_get_thread_num();
        //if (id != NTHREADS-1) {
        int begin = LOWER_LIMIT + (id+1) * band_size - max_distance;
        int end = LOWER_LIMIT + (id+1) * band_size + max_distance;
        //int begin = (id+1) * band_size - max_distance;
        //int end = (id+1) * band_size + max_distance;
        swapCells (begin, end, health_states_grid, pa_, f3_, RC_, move_proportion, max_distance);
        //}
        }
    }
...
void calculate_self_isolation_state_transitions (CELL_TYPE *health_states_grid, AC_TYPE *AC, int t) {
	...
	if (t>= 39) {
	    ...
	    #pragma omp parallel for private (i, j, qq)
		for (i=LOWER_LIMIT; i<UPPER_LIMIT; i++) {
			for (j=LOWER_LIMIT; j<UPPER_LIMIT; j++) {
				...
				qq = r4_uniform_01 (&seed[64/sizeof(int) *omp_get_thread_num()]);
...
int write_results_to_file () {
	...
	timenow = gmtime(&now);
	strftime(filename, sizeof(filename), "covid19_openmp_results_%Y-%m-%d_%H:%M:%S.txt", timenow);
...
unsigned int random_uint(unsigned int limit, unsigned int *seed) {
    ...
    // Función que implementa el generador LCG
